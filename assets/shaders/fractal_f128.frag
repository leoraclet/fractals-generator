...
// Emulated quadruple precision GLSL library created by Henry thasler
// (thasler.org/blog) based on the QD library
// (http://crd-legacy.lbl.gov/~dhbailey/mpdist/)

// Turn off nvidia's card optimizations
// Thanks to Cyril Crassin: https://www.icare3d.org/news_articles/nvidia_glsl_compiling_options.html
...
// ==================================================== //
// Functions to emulate operations on quad-single
// ==================================================== //
...
float qs_mandel(void) {
    ...
    vec4 cx =
        qs_add(qs_add(vec4(qs_cx), qs_mul(qs_tx, vec4(qs_z, 0., 0.))),
            vec4(qs_w, 0., 0.));
    vec4 cy =
        qs_add(qs_add(vec4(qs_cy), qs_mul(qs_ty, vec4(qs_z, 0., 0.))),
            vec4(qs_h, 0., 0.));
    ...
    for (int n = 0; n < iterations; n++) {
        ...
        if (qs_compare(qs_add(qs_mul(zx, zx), qs_mul(zy, zy)), e_radius) > 0.) {
            return float(n) + 1. -
                log(log(length(vec2(zx.x, zy.x)))) /
                    log(2.); // http://linas.org/art-gallery/escape/escape.html
        }
    }
...
float qs_julia()
{
    vec4 ci = vec4(cI, vec3(0.));
    vec4 cr = vec4(cR, vec3(0.));
    if (cI < 0.)
    {
        return cR;
    }
