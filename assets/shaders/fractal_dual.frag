...
float mandelbrot_d(dvec2 pos)
{
    dvec2 d_c = dvec2(cx, cy);
    dvec2 d_s = dvec2(sx, sy);

    dvec2 c = d_c + pos*d_z + d_s;
    dvec2 z = c;
...
float mandelbrot_f(vec2 pos)
{
    vec2 d_c = vec2(cx, cy);
    vec2 d_s = vec2(sx, sy);

    vec2 c = d_c + pos*float(d_z) + d_s;
    vec2 z = c;

    for(int n = 0; n < iterations; n++)
    {
        z = vec2(z.x*z.x - z.y*z.y, 2.0lf*z.x*z.y) + c;
        if(length(vec2(z.x,z.y)) > 4.0)
        {
            return float(n) + 1. - log(log(length(vec2(z.x,z.y))))/log(2.);
        }
...
float julia_d(dvec2 pos)
{
    dvec2 d_c = dvec2(cx, cy);
    dvec2 d_s = dvec2(sx, sy);

    dvec2 z = d_c + pos*d_z + d_s;
    dvec2 c = dvec2(cR, cI);
...
float julia_f(vec2 pos)
{
    vec2 d_c = vec2(cx, cy);
    vec2 d_s = vec2(sx, sy);

    vec2 z = d_c + pos*float(d_z) + d_s;
    vec2 c = vec2(cR, cI);
...
vec4 newton_1()
{
    // TODO: Newton 1 (double and float)
...
vec4 newton_2()
{
    // TODO: Newton 2 (double and float)
    return vec4(0.0, 0.0, 0.0, 1.0);
}
...
void main()
{
    ...
    dvec2 pos_d = dvec2(gl_FragCoord.xy);
    vec2 pos_f = vec2(gl_FragCoord.xy);

    if (gl_FragCoord.x / width < 0.497)
    {
    ...
    else if (gl_FragCoord.x / width > 0.503)
    {
        // Which fractal to draw
        switch (fractal)
        {
